...
install_vim () {
    if [[ -f ".vimrc" ]]; then
        mv ".vimrc" ".vimrc_back"
        success "Backup .vimrc to .vimrc_back"
    fi

    if [[ -d ".vim" ]]; then
        if [[ "$(readlink .vim)" =~ \.SpaceVim$ ]]; then
            success "Installed SpaceVim for vim"
        else
            mv ".vim" ".vim_back"
            success "BackUp .vim to .vim_back"
            ln -s ".SpaceVim" ".vim"
            success "Installed SpaceVim for vim"
        fi
    else
...
install_package_manager () {
    if [[ ! -d ".cache/vimfiles/repos/github.com/Shougo/dein.vim" ]]; then
        ...
        git clone https://github.com/Shougo/dein.vim.git .cache/vimfiles/repos/github.com/Shougo/dein.vim
...
install_neovim () {
    if [[ -d ".config/nvim" ]]; then
        if [[ "$(readlink .config/nvim)" =~ \.SpaceVim$ ]]; then
            success "Installed SpaceVim for neovim"
        else
            mv ".config/nvim" ".config/nvim_back"
            success "BackUp .config/nvim to .config/nvim_back"
...
uninstall_vim () {
    if [[ -d ".vim" ]]; then
        if [[ "$(readlink .vim)" =~ \.SpaceVim$ ]]; then
            ...
            if [[ -d ".vim_back" ]]; then
                mv ".vim_back" ".vim"
                success "Recover from .vim_back"
            fi
        fi
    fi
    if [[ -f ".vimrc_back" ]]; then
        mv ".vimrc_back" ".vimrc"
        success "Recover from .vimrc_back"
    fi
}
# }}}
...
uninstall_neovim () {
    if [[ -d ".config/nvim" ]]; then
        if [[ "$(readlink .config/nvim)" =~ \.SpaceVim$ ]]; then
            ...
            if [[ -d ".config/nvim_back" ]]; then
                mv ".config/nvim_back" ".config/nvim"
                success "Recover from .config/nvim_back"
            fi
        fi
    fi
}
